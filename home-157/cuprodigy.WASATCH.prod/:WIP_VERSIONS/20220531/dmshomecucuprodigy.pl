...
$CUSTOM{LIST}=join("\t","custom_TRN_MM_prioritized.pi","custom_baldesc.pi","custom_block.pi","custom_micr.pi","custom_parsing_mir.pi","custom_password.pi","custom_prehistory.pi","custom_preproc.pi","custom_sso.pi","custom_xmlrequest_memberpwd.pi","custom_xxxhistory_sortkey.pi");
...
sub inquiry{
   ...
   local($cuprodigy_method_used);
	...
	if(${failure_text} eq ""){
		...
		# Note that because CUProdigy does allow DMS/HomeCU to specify a date range for transaction history, the $CONF__MINDATE_VALID_CUPRODIGY_HISTORY is handled as part of the query (in the subroutine cuprodigy_xml_history()) instead of as a filter after the query (here in subroutine inquiry()).
	}
	if(${failure_text} eq ""){
		if(${CONF__CORE_DEGRADATION_CHECK__MIN_ROWS_PER_SECOND} > 0){
   ...
   local($MAX_ARCHIVE_COPIES)=15;
	...
	if    ($action=~/^START$/i){
				...
				for($ext=${max_archive_ext}-1;$ext>=0;$ext=sprintf("%.0f",${ext}-1)){
					...
					if(-f "${filename}.${archive_ext_newer}.raw"){
						if(!rename("${filename}.${archive_ext_newer}.raw","${filename}.${archive_ext_older}.raw")){
							&logfile("WARNING: rawdata_for_mbnum(): Failed renaming '${filename}.${archive_ext_newer}.raw' to '${filename}.${archive_ext_older}.raw'.\n");
						}
					}
				}
			}
		...
		if(!rename(${filename_wip},${filename})){
			&logfile("WARNING: rawdata_for_mbnum(): Failed renaming '${filename_wip}' to '${filename}'.\n");
		}
		if(!rename("${filename_wip}.raw","${filename}.raw")){
			&logfile("WARNING: rawdata_for_mbnum(): Failed renaming '${filename_wip}.raw' to '${filename}.raw'.\n");
		}
	...
	if(${rtrn_error_text} eq ""){
		...
		if(${rtrn_error_text} eq ""){
			...
			if(${error} ne ""){
				...
				&logfile("cuprodigy_xml_initial_password(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: ValidatePassword: ${error}\n");
				$rtrn_error_text=join("\t","001",$CTRL__STATUS_TEXT{"001"});
			}
		}
		if(${rtrn_error_text} eq ""){
			...
			if(${error} ne ""){
				...
				&logfile("cuprodigy_xml_initial_password(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: ValidatePassword: Response: ${error}\n");
				$rtrn_error_text=join("\t","001",$CTRL__STATUS_TEXT{"001"});
			}
		}
	...
	if(${rtrn_error_text} eq ""){
		...
		if(${rtrn_error_text} eq ""){
			...
			if(${error} ne ""){
				...
				&logfile("cuprodigy_xml_initial_password(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: GetMemberAutoEnrollInfo: ${error}\n");
				$rtrn_error_text=join("\t","001",$CTRL__STATUS_TEXT{"001"});
			}
		}
		if(0){
			...
			if(${rtrn_error_text} eq ""){
				...
				if(${error} ne ""){
					...
					&logfile("cuprodigy_xml_initial_password(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: GetMemberAutoEnrollInfo: Response: ${error}\n");
					$rtrn_error_text=join("\t","001",$CTRL__STATUS_TEXT{"001"});
				}
			}
		}
		if(${rtrn_error_text} eq ""){
			if($XML_DATA_BY_TAG_INDEX{${XML_KEY_SOAP_BODY},"submitMessageResponse",${XML_SINGLE},"return",${XML_SINGLE},"response",${XML_SINGLE},"memberInformation",${XML_SINGLE},"memberNumber",${XML_SINGLE}} ne ${cuprodigy_xml_request_membernumber}){
				&logfile("cuprodigy_xml_initial_password(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: GetMemberAutoEnrollInfo: Failure in XML body <submitMessageResponse><return><response><memberInformation> where <memberNumber> value does not match expected value of: ${cuprodigy_xml_request_membernumber}\n");
				$rtrn_error_text=join("\t","001",$CTRL__STATUS_TEXT{"001"});
			}
		}
...
sub cuprodigy_xml_balances_and_history__parse_history{
   ...
   local($tag_L10,$idx_L10,$limit_L10,$key_L10);
	...
	#	@XML_MB_LN_UNIQID
	#	@XML_MB_CC_UNIQID
	#	@XML_MB_DP_BALS, @XML_MB_DP_GROUPS
	#	@XML_MB_LN_BALS, @XML_MB_LN_GROUPS
	#	@XML_MB_CC_BALS, @XML_MB_CC_GROUPS, %XML_MB_CC_TO_UNIQ, %XML_MB_CC_FROM_UNIQ
	#
	# Will populate (calling routine must have declared as "local()"):
	#	@XML_MB_DP_HIST
	#	@XML_MB_LN_HIST
									...
									if(${hist_class} eq "CONF__HIS_EXCL_LIST_ACCTTYPE_DP" or ${hist_class} eq "CONF__HIS_EXCL_LIST_ACCTTYPE_LN"){
										...
										if(${cuprodigy_accountNumber__mb} eq ${cuprodigy_xml_request_membernumber} or !${CTRL__CUPRODIGY_GLITCH__HOLDS__INCLUDES_OTHER_MEMBERS_PENDINGS_FOR_CROSS_ACCOUNT_TRANSFERS}){
											if($KNOWN_EXPIRED_HIST_CLASS{${hist_mbnum},${hist_mbnum},${hist_qualifier1},${hist_qualifier2}} ne ""){
												...
												if(${hist_class} ne ""){
													&logfile("cuprodigy_xml_history(): Not coded to handle \$hist_class value '".${hist_class}."' for ${hist_mbnum}/${hist_qualifier1}/${hist_qualifier2}.\n") if ${record_messages_in_logfile};
												}else{
													&logfile("cuprodigy_xml_history(): Unable to determine a \$hist_class value (having checked \%KNOWN_HIST_CLASS and checked \%KNOWN_EXPIRED_HIST_CLASS and tested for existance of values ${hist_class_tested_for}) for ${hist_mbnum}/${hist_qualifier1}/${hist_qualifier2}.\n") if ${record_messages_in_logfile};
												}
											}
										}
									}
			...
			if($beg_ccyymmdd_xx =~ /^\d{8}$/ and $prehistory_overlap_date =~ /^\d{8}$/ and ${beg_ccyymmdd_xx} le ${prehistory_overlap_date}){
				...
				if(${hist_class} eq "CC"){
					if(${CTRL__CUPRODIGY_HAS_CC_HIST}){
						if(${CTRL__SHORTEN_CC_TO_LAST_4_DIGITS}){
							&logfile("cuprodigy_xml_history(): Can not handle CC pre-history for ${hist_mbnum}/${hist_cc} because the short CC number values may change (hence causing problems with already loaded history).\n") if ${record_messages_in_logfile};
							...
							for($tag_L06="holds",$idx_L06=1,$limit_L06=$XML_DATA_BY_TAG_INDEX{join($;,@key_prefix,$tag_L06)};$idx_L06<=$limit_L06;$idx_L06++){
								...
								for($tag_L07="hold",$idx_L07=1,$limit_L07=$XML_DATA_BY_TAG_INDEX{join($;,@key_prefix,$tag_L07)};$idx_L07<=$limit_L07;$idx_L07++){
									...
									if(${hold_class} eq "DP" or ${hold_class} eq "LN"){	# Intentionally excluding '${hold_class} eq "CC"' until I get an actual example of the XML
										...
										if(${record_messages_in_logfile}){
											if(${cuprodigy_accountNumber__mb} eq ${cuprodigy_xml_request_membernumber} or !${CTRL__CUPRODIGY_GLITCH__HOLDS__INCLUDES_OTHER_MEMBERS_PENDINGS_FOR_CROSS_ACCOUNT_TRANSFERS}){
												&logfile("cuprodigy_xml_history(): Not coded to handle \$hold_class value '".${hold_class}."' for ${hold_mbnum}/${hold_qualifier1}/${hold_qualifier2}.\n") if ${hold_class} ne "";	# The %ENCOUNTERED_UNKNOWN_HOLD_CLASS has already been populated to handle (via logfile()) when $hold_class is "";
											}
										}
									}
									pop(@key_prefix); pop(@key_prefix);
...
sub cuprodigy_xml_balances_and_history__parse_plastic_cards{
   ...
   local($tag_L10,$idx_L10,$limit_L10,$key_L10);
	...
	if(!${XML_has_branch_cardInfo}){
		if(${rtrn_error_text} eq ""){
			$rtrn_error_text=join("\t","999",${CTRL__ERROR_999_PREFIX__CUPRODIGY}."cuprodigy_xml_balances_and_history__parse_plastic_cards(): No XML branch <cardInfo>.");
		}
	}
	return(${rtrn_error_text});
}
   ...
   local($seq,$seq_key_prefix);
	...
	if(${rtrn_error_text} eq ""){
		...
		if(${rtrn_error_text} eq ""){
			$error=&validate_Body_message_error_RS("");
			if(${error} ne ""){
				&logfile("cuprodigy_xml_loanapplicationstatus(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: GetVendorLoanTypes: ${error}\n");
...
sub cuprodigy_xml_lookup_plastic_card_from_signature{
   ...
   local($seq,$seq_key_prefix);
	...
	if(!${XML_has_branch_cardInfo}){
		$rtrn_error_text=${CTRL__ERROR_999_PREFIX__CUPRODIGY}."cuprodigy_xml_lookup_plastic_card_from_signature(): Method \"${using_cuprodigy_method}\" contains no XML branch <cardInfo>.";
	}
...
sub cuprodigy_xml_get_plastic_card_attached{
   ...
   local($cuprodigy_fundingAccount,$cuprodigy_accountNumber,$cuprodigy_accountType,$cuprodigy_accountCategory);
	...
	if(!${XML_has_branch_cardInfo}){
		if(${rtrn_error_text} eq ""){
			$rtrn_error_text=join("\t","999",${CTRL__ERROR_999_PREFIX__CUPRODIGY}."cuprodigy_xml_get_plastic_card_attached(): Method \"${using_cuprodigy_method}\" contains no XML branch <cardInfo>.");
		}
	}
   ...
   my($auth_request_membernumber_ref);
	...
	if(${rtrn_error_text} eq ""){
		...
		if(${rtrn_error_text} eq ""){
			...
			if(${error} ne ""){
				&logfile("cuprodigy_xml_member_transfer(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: AccountTransfer: Response: ${error}\n");
   ...
   my($auth_request_membernumber_ref);
	...
	if(${rtrn_error_text} eq ""){
		...
		if(${rtrn_error_text} eq ""){
			...
			if(${error} ne ""){
				&logfile("cuprodigy_xml_member_transfer(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: LoanPayment: Response: ${error}\n");
   ...
   my($auth_request_membernumber_ref);
	...
	if(${rtrn_error_text} eq ""){
		...
		if(${rtrn_error_text} eq ""){
			...
			if(${error} ne ""){
				&logfile("cuprodigy_xml_member_transfer(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: LoanAddon: Response: ${error}\n");
   ...
   my($auth_request_membernumber_ref);
	...
	if(${rtrn_error_text} eq ""){
		...
		if(${rtrn_error_text} eq ""){
			...
			if(${error} ne ""){
				&logfile("cuprodigy_xml_member_transfer(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: CheckWithdrawal: Response: ${error}\n");
   ...
   my($auth_request_membernumber_ref);
	...
	if(${rtrn_error_text} eq ""){
		...
		if(${rtrn_error_text} eq ""){
			...
			if(${error} ne ""){
				&logfile("cuprodigy_xml_member_transfer(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: CheckWithdrawalLoan: Response: ${error}\n");
   ...
   my($auth_request_membernumber_ref);
	...
	if(${rtrn_error_text} eq ""){
		...
		if(${rtrn_error_text} eq ""){
			...
			if(${error} ne ""){
				&logfile("cuprodigy_xml_member_transfer(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: CreditCardPayment: Response: ${error}\n");
   ...
   local($cuprodigy_estatementactive);
	...
	if(${rtrn_error_text} eq ""){
		...
		if(${rtrn_error_text} eq ""){
			$error=&validate_Body_message_transaction_RS("");
			if(${error} ne ""){
				&logfile("cuprodigy_xml_member_estatementinquiry(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: AccountInquiry: Response: ${error}\n");
   ...
   my($error);
	...
	if(${rtrn_error_text} eq ""){
		...
		if(${rtrn_error_text} eq ""){
			$error=&validate_Body_message_transaction_RS("");
			if(${error} ne ""){
				&logfile("cuprodigy_xml_member_estatementchange(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: EStatementActivation: Response: ${error}\n");
   ...
   my($masked_plastic_card_pan) = '*' x length(${plastic_card_pan})-4 . substr(${plastic_card_pan},-4,4);
	...
	if(${rtrn_error_text} eq ""){
		...
		if(${rtrn_error_text} eq ""){
			$error=&validate_Body_message_transaction_RS("");
			if(${error} ne ""){
				&logfile("cuprodigy_xml_member_changecardstatus(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: ChangeCardStatus: Response: ${error}\n");
	...
	if(${rtrn} == 1){
		if(${rtrn_error_text} eq ""){
			...
			if(${rtrn_error_text} eq ""){
				...
				if(${error} ne ""){
					&logfile("cuprodigy_xml_member_transfer(): ".${CTRL__ERROR_999_PREFIX__CUPRODIGY}."Failed using method: UpdateInfo: Response: ${error}\n");
...
sub xxx_response_notes_print_STDOUT_as_XML{
   ...
   local($note,@lines);
   local($orig_htmlfilter_mode_POSTGRES);
   local(@fields,$idx);
			...
			while(@lines > 0){
				...
				if($lines[0] ne ""){
					@fields=split(/\t/,$lines[0]);
	   				$orig_htmlfilter_mode_POSTGRES=${htmlfilter_mode_POSTGRES};
					$htmlfilter_mode_POSTGRES=0;
					for($idx=0;$idx<=$#v;$idx++){
						$fields[0]=&htmlfilter($fields[0]);
					}
   					$htmlfilter_mode_POSTGRES=${orig_htmlfilter_mode_POSTGRES};
					$lines[0]=join("\t",@fields);
